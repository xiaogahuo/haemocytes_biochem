function [UU, W, Z, Zi, iter, obj] = algo_qp(X, Y, A, AA, numanchor, beta)
    ...
    numsample = size(Y, 1);  % Number of samples
    k=7;  %需要一直修改   % Number of clusters    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while flag
        ...
        if (iter > 1) && (abs((obj(iter - 1) - obj(iter)) / obj(iter - 1)) < 1e-3 || iter > maxIter || obj(iter) < 1e-10)
            ...
            [UU, ~, ~] = mySVD(Z', k);  % Final SVD to get UU  %k的作用  这里是跟聚类数一样
            %K 在mySVD中即为 ReducedDim，它决定了最终保留的奇异值和奇异向量的数量。
            % 如果你希望通过奇异值分解来进行降维，K 决定了要保留的奇异值数量以及降维后的维度大小。
            % 小 𝐾：保留较少的奇异值，可以进行较大幅度的降维，减少计算复杂度，适合特征冗余多的情况;
            % 大 𝐾：保留较多的奇异值，保留更多的信息，但降维效果较差。
