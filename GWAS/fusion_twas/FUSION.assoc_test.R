...
source(here("./fusion_twas/utils","plink_utils.R"))
...
wgtlist$WGT <- gsub("\r", "", wgtlist$WGT, fixed = TRUE)
...
if ( !is.na(opt$coloc_P) ) {
	...
	cat("INFormation: out.tbl$COLOC.PP0 = as.numeric(rep(NA,N))", N, "\n")
...
# 				    print(gwas.beta)
				...
				# 清洗 d1/d2 中的 NA：只保留两个 dataset 中 beta 和 varbeta 均非 NA 的 SNP
				valid.idx = which(!is.na(d1$beta) & !is.na(d1$varbeta) & !is.na(d2$beta) & !is.na(d2$varbeta))
				if ( length(valid.idx) < 2 ) {
					cat("WARNING : Too few valid SNPs for COLOC after removing NAs (", length(valid.idx), "), skipping.\n")
				} else {
					d1$beta = d1$beta[valid.idx]
					d1$varbeta = d1$varbeta[valid.idx]
					d1$LD = cur.LD[valid.idx, valid.idx, drop=FALSE]
					d1$snp = d1$snp[valid.idx]
					d1$position = d1$position[valid.idx]

					d2$beta = d2$beta[valid.idx]
					d2$varbeta = d2$varbeta[valid.idx]
					d2$LD = cur.LD[valid.idx, valid.idx, drop=FALSE]
					d2$snp = d2$snp[valid.idx]
					d2$position = d2$position[valid.idx]
					cat("INFORMATION: COLOC running on", length(valid.idx), "valid SNPs\n")
				}
...
# 					print(e)
				...
				print(clc)
				if ( !is.null(clc) ) {
					 cat( "INFORMATION : set out.tbl$COLOC.PP0[w].\n")
